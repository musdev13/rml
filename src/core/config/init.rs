...
pub fn init(force: bool){
    ...
    if force {
        println!("!!! FORCED !!!");
        musutils::fs::config::rewrite("rmlaunch", "paths.json", &def_config_str);
    } else {
        musutils::fs::config::get("rmlaunch", "paths.json", Some(&def_config_str));
    }
    println!("done :3\n\n!!! check your ~/.config or %AppData% directory for rmlaunch configuration !!!");
}
